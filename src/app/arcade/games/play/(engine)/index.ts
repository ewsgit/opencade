...
export default class Engine {
    ...
    constructor(options: IEngineOptions, callback: (engine: Engine) => void) {
        ...
        if (!this.options.containerElement) throw new Error(`${ENGINE_LOG_PREFIX} The engine option 'containerElement' must be assigned to a HTML 'div' element.`)
        ...
        window.engineResizeObserver = new ResizeObserver(() => {
            ...
            resizeObserverWaitTimeout = setTimeout(() => {
                this.layers.forEach(layer => {
                    ...
                    if (layer.canvas.width !== containerRect.width || layer.canvas.height !== containerRect.height) {
        ...
        window.engineMutationObserver = new MutationObserver(() => {
            if ((container.parentElement!.getAttribute("data-devmode") === "true") !== this.DEV_MODE) console.log(`${ENGINE_LOG_PREFIX}DevMode: ${!this.DEV_MODE ? "enabled" : "disabled"}`)
...
function drawLoadingScreen(context: CanvasRenderingContext2D, engine: Engine, logo: HTMLImageElement) {
    ...
    context.fillRect(containerMargin, containerMargin, engine.screen()
                                                             .width() - (containerMargin * 2), engine.screen()
                                                                                                     .height() - (containerMargin * 2))
    ...
    let centerContainerHeight = engine.screen()
                                      .height() / 4

    context.drawImage(logo, (engine.screen()
                                   .width() / 2) - (256 / 2), centerContainerHeight, 256, 256)

    context.fillText("OpenCade", engine.screen()
                                       .width() / 2, (centerContainerHeight + 256) + containerMargin)
}
